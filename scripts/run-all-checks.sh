#!/usr/bin/env bash
#
# rms-spyceman - Run All Checks Script
#
...
#     ENABLE_STUBTEST     .pyi stubs match the runtime API (default: false; this
#                         repo has no stubs)
...
: "${ENABLE_PYTEST:=true}"
: "${ENABLE_PYROMA:=true}"
# No .pyi stubs in this repo (the source carries no type annotations), so there is
# nothing for stubtest to check. Turn this on if stubs are ever added.
: "${ENABLE_STUBTEST:=false}"
: "${ENABLE_BANDIT:=false}"
...
print_header "rms-spyceman - Running All Checks"
...
run_code_checks() {
    ...
    if [ "$RUN_STUBTEST" = true ] && [ "$ENABLE_STUBTEST" = true ]; then
        ...
        if python -m mypy.stubtest spyceman --mypy-config-file pyproject.toml; then
