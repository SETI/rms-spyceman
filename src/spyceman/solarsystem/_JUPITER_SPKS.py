##########################################################################################
# spyceman/solarsystem/_JUPITER_SPKS.py
##########################################################################################
"""Managed list of Jupiter satellite SPK files, complete as of 2/1/24."""

from spyceman.kernelfile import KTuple

_JUPITER_SPKS = [
    KTuple('jup068.bsp',
        '1995-01-01T00:00:00', '2002-01-02T23:59:57',
        {3, 5, 10, 399, 506, 507, 508, 509, 510, 511, 512, 513, 599},
        '1995-01-29'),
    KTuple('jup069.bsp',
        '1995-01-01T00:00:00', '2002-01-03T12:00:00',
        {3, 5, 10, 399, 506, 507, 508, 509, 510, 511, 512, 513, 599},
        '1999-02-08'),
    KTuple('jup073-hst.bsp',
        '1996-01-01T00:00:00', '1999-12-31T23:59:58',
        {3, 5, 10, 301, 399, 501, 502, 503, 504, 505, 506, 507, 508, 509, 510, 511, 512,
         513, 514, 515, 516, 599},
        '1996-03-16'),
    KTuple('jup073s.bsp',
        '1995-01-01T00:00:00', '2001-12-31T23:59:58',
        {3, 5, 10, 301, 399, 501, 502, 503, 504, 505},
        '1996-08-12'),
    KTuple('jup100.bsp',
        '1973-10-31T23:59:15.817', '2023-11-02T23:58:50.817',
        {3, 5, 10, 301, 399, 501, 502, 503, 504, 599},
        '1996-08-28'),
    KTuple('jup104.bsp',
        '1996-09-04T00:00:00', '2002-01-02T23:59:58',
        {3, 5, 10, 301, 399, 505, 514, 515, 516, 599},
        '1997-01-16'),
    KTuple('jup120_1996-2010.bsp',
        '1995-12-31T23:58:58.816', '2009-12-31T23:58:53.816',
        {3, 5, 10, 301, 399, 505, 514, 515, 516, 599},
        '1997-09-16'),
    KTuple('jup147s.bsp',
        '1988-01-01T00:00:00', '2002-01-05T00:00:00',
        {3, 5, 10, 399, 501, 502, 503, 504, 599},
        '1999-08-03'),
    KTuple('jup164_20year.bsp',
        '1999-12-31T23:58:55.816', '2020-12-31T23:58:50.816',
        {506, 507, 508, 509, 510, 511, 512, 513, 599},
        '2000-06-01'),
    KTuple('jup164.bsp',
        '1999-12-31T23:58:55.816', '2020-12-31T23:58:50.816',
        {506, 507, 508, 509, 510, 511, 512, 513, 599},
        '2000-06-01'),
    KTuple('jup166.bsp',
        '1999-12-31T23:58:55.816', '2023-10-30T23:58:50.818',
        {3, 5, 10, 399, 501, 502, 503, 504, 599},
        '2000-10-25'),
    KTuple('jup172.bsp',
        '1999-12-31T23:58:55.816', '2023-10-30T23:58:50.818',
        {3, 5, 10, 399, 506, 507, 508, 509, 510, 511, 512, 513, 599},
        '2000-07-25'),
    KTuple('jup202_2000_2010.bsp',
        '1999-12-31T23:58:55.816', '2009-12-31T23:58:53.816',
        {3, 5, 10, 399, 517, 518, 519, 520, 521, 522, 523, 524, 525, 526, 527, 528, 599},
        '2002-01-24'),
    KTuple('jup204.bsp',
        '1924-12-28T23:59:18.816', '2024-12-31T23:58:50.816',
        {501, 502, 503, 504, 505, 514, 515, 516, 599},
        '2002-05-20'),
    KTuple('jup214.bsp',
        '1994-12-31T23:58:58.816', '2023-10-30T23:58:50.818',
        {3, 5, 10, 399, 517, 518, 519, 520, 521, 522, 523, 524, 525, 526, 527, 599},
        '2002-11-04'),
    KTuple('jup215.bsp',
        '1994-12-31T23:58:58.816', '2023-08-03T23:58:50.817',
        {3, 5, 10, 399, 529, 530, 531, 532, 533, 534, 535, 536, 537, 538, 539, 599},
        '2003-01-16'),
    KTuple('jup219.bsp',
        '1994-12-31T23:58:58.816', '2023-10-22T23:58:50.818',
        {3, 5, 10, 399, 540, 599},
        '2003-01-24'),
    KTuple('jup230-rocks.bsp',
        '1999-12-31T23:58:55.816', '2050-01-01T23:58:50.816',
        {515, 516},
        '2007-03-30'),
    KTuple('jup230-short.bsp',
        '1999-12-31T23:58:55.816', '2049-12-31T23:58:50.816',
        {3, 5, 10, 399, 501, 502, 503, 504, 505, 514, 599},
        '2004-01-08'),
    KTuple('jup230l.bsp',
        '1900-01-01T00:00:00', '2100-01-01T23:59:57',
        {3, 5, 10, 399, 501, 502, 503, 504, 505, 514, 599},
        '2010-01-19'),
    KTuple('jup230.bsp',
        '1900-01-01T00:00:00', '2100-01-01T23:59:57',
        {3, 5, 10, 399, 501, 502, 503, 504, 505, 514, 599},
        '2004-01-08'),
    KTuple('jup251.bsp',
        '1975-08-16T00:00:00', '2023-07-31T23:59:56',
        {3, 5, 10, 399, 517, 518, 519, 520, 521, 522, 523, 524, 525, 526, 527, 599},
        '2004-11-29'),
    KTuple('jup252.bsp',
        '2001-11-27T00:00:00', '2023-07-31T23:59:56',
        {3, 5, 10, 399, 528, 529, 530, 531, 532, 533, 534, 535, 536, 537, 538, 599},
        '2004-11-29'),
    KTuple('jup253.bsp',
        '2002-01-07T00:00:00', '2023-09-11T23:59:56',
        {3, 5, 10, 399, 540, 541, 542, 543, 544, 545, 546, 547, 548, 549, 599},
        '2004-11-29'),
    KTuple('jup254.bsp',
        '2002-01-07T00:00:00', '2023-09-11T23:59:56',
        {3, 5, 10, 399, 550, 551, 552, 553, 554, 555, 556, 557, 558, 559, 599},
        '2004-11-29'),
    KTuple('jup255.bsp',
        '2002-01-07T00:00:00', '2023-09-11T23:59:56',
        {3, 5, 10, 399, 560, 561, 599, 55062, 55063},
        '2004-11-29'),
    KTuple('jup256.bsp',
        '1972-01-01T00:00:00', '2049-12-31T23:59:56',
        {3, 5, 10, 399, 506, 507, 508, 509, 510, 511, 512, 513, 599},
        '2006-10-18'),
    KTuple('jup261.bsp',
        '1975-08-16T00:00:00', '2049-11-13T23:59:56',
        {3, 5, 10, 399, 517, 518, 519, 520, 521, 522, 523, 524, 525, 526, 527, 599},
        '2007-01-03'),
    KTuple('jup262.bsp',
        '2002-01-06T00:00:00', '2049-12-23T23:59:56',
        {3, 5, 10, 399, 539, 540, 541, 542, 543, 544, 545, 546, 547, 548, 599},
        '2007-01-03'),
    KTuple('jup263.bsp',
        '2002-01-06T00:00:00', '2049-12-23T23:59:56',
        {3, 5, 10, 399, 599, 55049, 55050, 55051, 55052, 55053, 55054, 55055, 55056,
         55057, 55058, 55059, 55060, 55061, 55063},
        '2007-01-03'),
    KTuple('jup266.bsp',
        '1950-01-01T00:00:00', '2049-12-31T23:59:57',
        {3, 5, 10, 399, 539, 540, 541, 542, 543, 544, 545, 546, 547, 548, 549, 599},
        '2009-09-24'),
    KTuple('jup267.bsp',
        '1950-01-01T00:00:00', '2049-12-31T23:59:57',
        {3, 5, 10, 399, 599, 55050, 55051, 55052, 55053, 55055, 55056, 55057, 55058,
         55059, 55060, 55061, 55063, 55064},
        '2009-09-24'),
    KTuple('jup268.bsp',
        '1950-01-01T00:00:00', '2049-12-31T23:59:57',
        {3, 5, 10, 399, 539, 540, 541, 542, 543, 544, 545, 546, 547, 548, 549, 599},
        '2009-12-17'),
    KTuple('jup269.bsp',
        '1950-01-01T00:00:00', '2049-12-31T23:59:57',
        {3, 5, 10, 399, 528, 529, 530, 531, 532, 533, 534, 535, 536, 537, 538, 599},
        '2009-12-17'),
    KTuple('jup270.bsp',
        '1950-01-01T00:00:00', '2049-12-31T23:59:57',
        {3, 5, 10, 399, 550, 599, 55060, 55061, 55062, 55063, 55064, 55065, 55066, 55067,
         55068, 55069, 55070, 55071},
        '2009-12-17'),
    KTuple('jup282-1.bsp',
        '1900-01-22T00:00:00', '2099-12-01T23:59:57',
        {3, 5, 10, 399, 506, 507, 508, 509, 510, 511, 512, 513, 517, 518, 519, 520, 521,
         522, 523, 524, 525, 526, 527, 528, 529, 530, 531, 532, 533, 534, 535, 536, 537,
         538, 539, 540, 541, 542, 543, 544, 545, 546, 547, 548, 549, 550, 599, 55060,
         55061, 55062, 55063, 55064, 55065, 55066, 55067, 55068, 55069, 55070, 55071},
        '2012-10-25'),
    KTuple('jup282.bsp',
        '1900-01-22T00:00:00', '2099-12-01T23:59:57',
        {3, 5, 10, 399, 506, 507, 508, 509, 510, 511, 512, 513, 517, 518, 519, 520, 521,
         522, 523, 524, 525, 526, 527, 528, 529, 530, 531, 532, 533, 534, 535, 536, 537,
         538, 539, 540, 541, 542, 543, 544, 545, 546, 547, 548, 549, 550, 599, 55060,
         55061, 55062, 55063, 55064, 55065, 55066, 55067, 55068, 55069, 55070, 55071,
         55072, 55073},
        '2011-05-26'),
    KTuple('jup291.bsp',
        '1900-01-01T00:00:00', '2099-12-29T23:59:58',
        {3, 5, 10, 399, 599, 55074, 55075},
        '2012-02-03'),
    KTuple('jup293.bsp',
        '1900-01-18T00:00:00', '2099-12-17T23:59:58',
        {3, 5, 10, 399, 599, 55076},
        '2012-09-07'),
    KTuple('jup294.bsp',
        '1900-02-04T00:00:00', '2099-12-14T23:59:58',
        {3, 5, 10, 399, 599, 55072, 55073, 55074, 55075},
        '2012-10-25'),
    KTuple('jup300.bsp',
        '1900-01-12T00:00:00', '2099-12-31T23:59:58',
        {3, 5, 10, 399, 506, 507, 508, 509, 510, 511, 512, 513, 517, 518, 519, 520, 521,
         522, 523, 524, 525, 526, 527, 528, 529, 530, 531, 532, 533, 534, 535, 536, 537,
         538, 539, 540, 541, 542, 543, 544, 545, 546, 547, 548, 549, 550, 599, 55060,
         55061, 55062, 55063, 55064, 55065, 55066, 55067, 55068, 55069, 55070, 55071,
         55072, 55073, 55074, 55075, 55076},
        '2013-10-15'),
    KTuple('jup309.bsp',
        '1900-01-01T00:00:00', '2099-12-31T23:59:58',
        {501, 502, 503, 504, 505, 514, 515, 516, 599},
        '2013-09-11'),
    KTuple('jup310xl.bsp',
        '1600-01-09T23:59:18.816', '2600-01-04T23:58:50.816',
        {501, 502, 503, 504, 505, 599},
        '2016-06-29'),
    KTuple('jup310_2004.bsp',
        '2004-01-01T00:00:00', '2005-01-01T00:00:00',
        {3, 5, 10, 399, 501, 502, 503, 504, 505, 514, 515, 516, 599},
        '2013-11-25'),
    KTuple('jup310_2018.bsp',
        '2018-01-01T00:00:00', '2019-01-01T00:00:00',
        {3, 5, 10, 399, 501, 502, 503, 504, 505, 514, 515, 516, 599},
        '2013-11-25'),
    KTuple('jup310_2021.bsp',
        '2021-01-01T00:00:00', '2022-01-01T00:00:00',
        {3, 5, 10, 399, 501, 502, 503, 504, 505, 514, 515, 516, 599},
        '2013-11-25'),
    KTuple('jup310.bsp',
        '1900-01-01T00:00:00', '2099-12-31T23:59:58',
        {3, 5, 10, 399, 501, 502, 503, 504, 505, 514, 515, 516, 599},
        '2013-11-25'),
    KTuple('jup329.bsp',
        '1900-01-12T00:00:00', '2099-12-31T23:59:58',
        {3, 5, 10, 399, 551, 552, 553, 599},
        '2015-03-09'),
    KTuple('jup341.bsp',
        '1900-01-01T00:00:00', '2200-01-01T00:00:00',
        {3, 5, 10, 399, 506, 507, 508, 509, 510, 511, 512, 513, 517, 518, 519, 520, 521,
         522, 523, 524, 525, 526, 527, 528, 529, 530, 531, 532, 533, 534, 535, 536, 537,
         538, 539, 540, 541, 542, 543, 544, 545, 546, 547, 548, 549, 550, 551, 552, 553,
         554, 555, 556, 557, 558, 559, 599, 55060, 55061, 55062, 55064, 55065, 55066,
         55068, 55070, 55071, 55074},
        '2017-09-28'),
    KTuple('jup343.bsp',
        '1800-01-01T00:00:00', '2200-01-01T00:00:00',
        {3, 5, 10, 399, 506, 507, 508, 509, 510, 511, 512, 513, 517, 518, 519, 520, 521,
         522, 523, 524, 525, 526, 527, 528, 529, 530, 531, 532, 533, 534, 535, 536, 537,
         538, 539, 540, 541, 542, 543, 544, 545, 546, 547, 548, 549, 550, 551, 552, 553,
         554, 555, 556, 557, 558, 559, 560, 561, 562, 563, 564, 565, 566, 567, 568, 569,
         570, 571, 572, 599, 55501, 55502, 55503, 55504, 55505, 55506, 55507},
        '2019-11-06'),
    KTuple('jup344-s2003_j24.bsp',
        '1800-01-04T23:59:18.816', '2200-12-21T23:58:50.816',
        {3, 5, 10, 399, 599, 55508},
        '2021-11-22'),
    KTuple('jup344.bsp',
        '1799-12-26T23:59:18.816', '2200-01-04T23:58:50.816',
        {3, 5, 10, 399, 506, 507, 508, 509, 510, 511, 512, 513, 517, 518, 519, 520, 521,
         522, 523, 524, 525, 526, 527, 528, 529, 530, 531, 532, 533, 534, 535, 536, 537,
         538, 539, 540, 541, 542, 543, 544, 545, 546, 547, 548, 549, 550, 551, 552, 553,
         554, 555, 556, 557, 558, 559, 560, 561, 562, 563, 564, 565, 566, 567, 568, 569,
         570, 571, 572, 599, 55501, 55502, 55503, 55504, 55505, 55506, 55507},
        '2021-03-14'),
    KTuple('jup345.bsp',
        '1799-12-27T23:59:18.816', '2199-12-18T23:58:50.817',
        {3, 5, 10, 399, 599, 55509, 55510, 55511, 55512, 55513, 55514, 55515, 55516,
         55517, 55518, 55519, 55520},
        '2023-02-09'),
    KTuple('jup346.bsp',
        '1799-12-27T23:59:18.816', '2199-12-18T23:58:50.817',
        {3, 5, 10, 399, 599, 55509, 55510, 55511, 55512, 55513, 55514, 55515, 55516,
         55517, 55518, 55519, 55520, 55521, 55522, 55523},
        '2023-03-02'),
    KTuple('jup365_2027.bsp',
        '2027-01-01T00:00:00', '2028-01-01T00:00:00',
        {3, 5, 10, 399, 501, 502, 503, 504, 505, 514, 515, 516, 599},
        '2023-02-18'),
    KTuple('jup365.bsp',
        '1600-01-09T23:59:18.816', '2200-01-09T23:58:50.816',
        {3, 5, 10, 399, 501, 502, 503, 504, 505, 514, 515, 516, 599},
        '2021-03-14'),
]

##########################################################################################
